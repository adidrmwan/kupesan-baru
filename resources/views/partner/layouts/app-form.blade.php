...
<body>
    ...
    <script type="text/javascript">
      $('#provinces4').on('change', function(e){
        console.log(e);
        var province_id = e.target.value;
        $.get('/json-regencies4?province_id=' + province_id,function(data) {
          console.log(data);
          if (province_id == 1) {
            $('#regencies4').empty();
          ...
          $('#districts4').empty();
          $('#districts4').append('<option value="0" disable="true" selected="true">Pilih Kecamatan</option>');

          $('#villages4').empty();
          $('#villages4').append('<option value="0" disable="true" selected="true">Pilih Kelurahan</option>');
        
          $.each(data, function(index, regenciesObj){
            $('#regencies4').append('<option value="'+ regenciesObj.id +'">'+ regenciesObj.type_name +'</option>');
          })
        });
      });

      $('#regencies4').on('change', function(e){
        console.log(e);
        var regencies_id = e.target.value;
        $.get('/json-districts4?regencies_id=' + regencies_id,function(data) {
          console.log(data);
          $('#districts4').empty();
          $('#districts4').append('<option value="0" disable="true" selected="true">Pilih Kecamatan</option>');

          $.each(data, function(index, districtsObj){
            $('#districts4').append('<option value="'+ districtsObj.id +'">'+ districtsObj.name +'</option>');
          })
        });
      });

      $('#districts4').on('change', function(e){
        console.log(e);
        var districts_id = e.target.value;
        $.get('/json-village4?districts_id=' + districts_id,function(data) {
          console.log(data);
          $('#villages4').empty();
          $('#villages4').append('<option value="0" disable="true" selected="true">Pilih Kelurahan</option>');

          $.each(data, function(index, villagesObj){
            $('#villages4').append('<option value="'+ villagesObj.name +'">'+ villagesObj.name +'</option>');
          })
        });
      });
