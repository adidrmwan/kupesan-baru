...
class AlbumController extends Controller
{
    ...
    public function updateAlbum(Request $request)
    {
	    ...
	    if ($request->hasFile('album_img_1')) {
            //Found existing file then delete
            $foto_update = $album->album_img_1;
            if( File::exists(public_path('/album/' . $foto_update .'.jpeg' ))){
                File::delete(public_path('/album/' . $foto_update .'.jpeg' ));
            }
            elseif( File::exists(public_path('/album/' . $foto_update .'.jpg' ))){
                File::delete(public_path('/album/' . $foto_update .'.jpg' ));
            }
            elseif( File::exists(public_path('/album/' . $foto_update .'.png' ))){
                File::delete(public_path('/album/' . $foto_update .'.png' ));
            }
            elseif( File::exists(public_path('/album/' . $foto_update .'.JPEG' ))){
                File::delete(public_path('/album/' . $foto_update .'.JPEG' ));
            }
            elseif( File::exists(public_path('/album/' . $foto_update .'.JPG' ))){
                File::delete(public_path('/album/' . $foto_update .'.JPG' ));
            ...
            $foto = $request->file('album_img_1');
            $foto_name = $foto_update . '.' .$foto->getClientOriginalExtension();
            Image::make($foto)->save( public_path('/album/' . $foto_name ) );
            $user = Auth::user();
            $album = Album::where('user_id', $user->id)->first();
            $album->save();
